...
class NLinkArm:
    ...
    def update_joints(self, joint_angles):
        if (self.joint_limits and
                not self.joint_limits.is_within_limits(joint_angles)):
            msg = f"Updated joint angles {joint_angles} Beyond the limits."
    ...
    def update_points(self):
        for i in range(1, self.n_links + 1):
            angle_sum = np.sum(self.joint_angles[:i])
            self.points[i][0] = (
                self.points[i - 1][0] +
                self.link_lengths[i - 1] * np.cos(angle_sum)
            )
            self.points[i][1] = (
                self.points[i - 1][1] +
                self.link_lengths[i - 1] * np.sin(angle_sum)
            )
    ...
    def draw(self, ax, obstacles=[], ee_x=None, ee_y=None, goal_angles=None):
        ...
        for obstacle in obstacles:
            circle = plt.Circle(
                ...
                radius=0.5 * obstacle[2],
                fc='gray'
            )
            ax.add_patch(circle)

        # Start point (blue square)
        if ee_x and ee_y:
            square_size = 0.08  # Square size control
            square = Rectangle(
                (ee_x[0], ee_y[0]),
                square_size,
                square_size,
        ...
        # End point (yellow hexagon to simulate star)
        if ee_x and ee_y:
            star_size = 0.14
            star = RegularPolygon(
                (ee_x[-1], ee_y[-1]),
